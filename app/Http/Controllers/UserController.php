...
class UserController extends Controller {
	...
	public function getRegister() {
		...
		if(CNF_REGIST =='false') :
			if(\Auth::check()):
				 return Redirect::to('')->with('message',\App\Library\SiteHelpers::alert('success','Youre already login'));
			else:
	...
	public function postCreate( Request $request) {
		...
		if ($validator->passes()) {
			...
			return Redirect::to('user/login')->with('message',\App\Library\SiteHelpers::alert('success',$message));
		} else {
			return Redirect::to('user/register')->with('message',\App\Library\SiteHelpers::alert('error','The following errors occurred')
	...
	public function getActivation( Request $request  )
	{
		...
		if($num =='')
			return Redirect::to('user/login')->with('message',\App\Library\SiteHelpers::alert('error','Invalid Code Activation!'));

		$user =  User::where('activation','=',$num)->get();
		if (count($user) >=1)
		{
			\DB::table('tb_users')->where('activation', $num )->update(array('active' => 1,'activation'=>''));
			return Redirect::to('user/login')->with('message',\App\Library\SiteHelpers::alert('success','Your account is active now!'));
	...
	public function getLogin() {
		...
		if(\Auth::check())
		{
			return Redirect::to('')->with('message',\App\Library\SiteHelpers::alert('success','You are already logged in'));
	...
	public function postSignin( Request $request) {
...

        {
				if(\Auth::check())
				{
					...
					if($row->active =='0')
					{
						...
						if($request->ajax() == true )
						{
							...
							\Auth::logout();
							return Redirect::to('user/login')->with('message', \App\Library\SiteHelpers::alert('error','Your Account is not active'));
						}
					...
					} else if($row->active=='2')
					{
						...
						if($request->ajax() == true )
						{
							...
							// BLocked users
							\Auth::logout();
							return Redirect::to('user/login')->with('message', \App\Library\SiteHelpers::alert('error','Your Account is blocked'));
						}
					} else {
				...
				if($request->ajax() == true )
				{
					...
					return Redirect::to('user/login')
						->with('message', \App\Library\SiteHelpers::alert('error','Your username/password combination was incorrect'))
						->withInput();
				}
				...
				if($request->ajax() == true)
				{
					...
					return Redirect::to('user/login')
						->with('message', \App\Library\SiteHelpers::alert('error','The following  errors occurred'))
						->withErrors($validator)->withInput();
				}
	...
	public function postSavepassword( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			return Redirect::to('user/profile')->with('message', \App\Library\SiteHelpers::alert('success','Password has been saved!'));
		} else {
			return Redirect::to('user/profile')->with('message', \App\Library\SiteHelpers::alert('error','The following errors occurred')
			)->withErrors($validator)->withInput();
		}
	...
	public function postRequest( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			if($user->count() >=1)
			{
				...
				return Redirect::to('user/login')->with('message', \App\Library\SiteHelpers::alert('success','Please check your email'));

			} else {
				return Redirect::to('user/login?reset')->with('message', \App\Library\SiteHelpers::alert('error','Cant find email address'));
			}

		}  else {
			return Redirect::to('user/login?reset')->with('message', \App\Library\SiteHelpers::alert('error','The following errors occurred')
			)->withErrors($validator)->withInput();
		}
	}
	...
	public function getReset( $token = '')
	{
		...
		if($user->count() >=1)
		{
			...
			return Redirect::to('user/login')->with('message', \App\Library\SiteHelpers::alert('error','Cant find your reset code'));
		}
	...
	public function postDoreset( Request $request , $token = '')
	{
		...
		if ($validator->passes()) {
			...
			return Redirect::to('user/login')->with('message',\App\Library\SiteHelpers::alert('success','Password has been saved!'));
		} else {
			return Redirect::to('user/reset/'.$token)->with('message', \App\Library\SiteHelpers::alert('error','The following errors occurred')
			)->withErrors($validator)->withInput();
		}
	...
	public function getLogout() {
		...
		return Redirect::to('')->with('message', \App\Library\SiteHelpers::alert('info','Your are now logged out!'));
	}
	...
	function autoSignin($user)
	{
		...
		if(is_null($user)){
		  return Redirect::to('user/login')
				->with('message', \App\Library\SiteHelpers::alert('error','You have not registered yet '))
				->withInput();
		} else{
