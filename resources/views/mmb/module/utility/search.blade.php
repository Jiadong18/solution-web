...
@foreach ($tableForm as $t)
	@if($t['search'] =='1')
		<tr id="{{ $t['field'] }}" class="fieldsearch">
			<td>{!! \App\Library\SiteHelpers::activeLang($t['label'],(isset($t['language'])? $t['language'] : array())) !!} </td>
			<td >
			...
			<td id="field_{{ $t['field']}}">{!! \App\Library\SiteHelpers::transForm($t['field'] , $tableForm) !!}</td>
